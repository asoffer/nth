...
NoDestructor<Ends> expectation_result_handlers;
...
ExpectationResultHandlerRange RegisteredExpectationResultHandlers() {
  uintptr_t const *ptr;
  {
    absl::MutexLock lock(&expectation_result_handlers->mutex);
    HandlerNode const *node = expectation_result_handlers->head;
    if (node->count() == 0) {
