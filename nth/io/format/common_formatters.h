...
struct word_formatter {
  void format(writer auto& w, bool b) const {
  ...
  void format(writer auto& w, decltype(nullptr)) const {
    if constexpr (C == casing::upper) {
...
struct ascii_formatter {
  void format(writer auto& w, char c) const {
...
struct base_formatter {
  ...
  void format(writer auto& w, std::integral auto n) const {
...
struct pointer_formatter {
  void format(writer auto& w, void const* ptr) const {
    constexpr char hex[]                   = "0123456789abcdef";
