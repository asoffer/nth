#ifndef NTH_CONTAINER_FLYWEIGHT_MAP_H
#define NTH_CONTAINER_FLYWEIGHT_MAP_H
...
template <typename K, typename M, nth::hasher<K> Hash = absl::Hash<K>,
...
struct flyweight_map {
  ...
  std::pair<iterator, bool> try_emplace(T&& t, Args&&... args) requires(
    ...
    if (auto set_iter = set_.find(t); set_iter != set_.end()) {
      ...
      values_.emplace_back(std::piecewise_construct, std::forward_as_tuple(t),
  ...
  iterator find(T const& t) requires(nth::hasher<hasher, T>) {
 ...
 private:
  struct H : private hasher {
    ...
    size_t operator()(internal_container::Index index) const {
...
#endif  // NTH_CONTAINER_FLYWEIGHT_MAP_H
